...
void LoopClosing::Run() {
  ...
  while (true) {
    ...
    if (CheckNewKeyFrames()) {
      ...
      if (bFindedRegion) {
        if (mbMergeDetected) {
          if ((mpTracker->mSensor == System::Sensor::IMU_MONOCULAR
               || mpTracker->mSensor == System::Sensor::IMU_STEREO
               || mpTracker->mSensor == System::Sensor::IMU_RGBD)
...
bool LoopClosing::DetectAndReffineSim3FromLastKF(
  ...
  if (nNumProjMatches >= nProjMatches) {
    ...
    if (mpTracker->mSensor == System::Sensor::IMU_MONOCULAR && !pCurrentKF->GetMap()->GetIniertialBA2()) {
      bFixedScale = false;
    }
...
void LoopClosing::CorrectLoop() {
  ...
  if (mpTracker->mSensor == System::Sensor::IMU_MONOCULAR && !mpCurrentKF->GetMap()->GetIniertialBA2()) {
    bFixedScale = false;
  }
...
void LoopClosing::MergeLocal() {
  ...
  if (mpTracker->mSensor == System::Sensor::IMU_MONOCULAR || mpTracker->mSensor == System::Sensor::IMU_STEREO
      || mpTracker->mSensor == System::Sensor::IMU_RGBD) {
  ...
  if (vpCurrentMapKFs.empty()) {
    ...
    if (mpTracker->mSensor == System::Sensor::MONOCULAR) {
      const std::unique_lock<std::mutex> currentLock(
        pCurrentMap->mMutexMapUpdate
      ); // We update the current map with the Merge information
