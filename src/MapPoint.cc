...
MapPoint::MapPoint(const Eigen::Vector3f& Pos, Map* pMap, Frame* pFrame, int idxF)
  : mnFirstKFid(-1)
  ...
  Eigen::Vector3f Ow;
  if (!pFrame->isDualCamera() || idxF < pFrame->Nleft) {
  ...
  const int             level            = (!pFrame->isDualCamera()) ? pFrame->mvKeysUn[idxF].octave
...
void MapPoint::AddObservation(KeyFrame* pKF, int idx) {
  ...
  if (pKF->isDualCamera() && idx >= pKF->NLeft) {
...
void MapPoint::PostLoad(
  ...
  for (const auto& [id, leftIdx] : mBackupObservationsId1) {
    ...
    const auto indexes = std::make_tuple(leftIdx, it2->second);
    if (pKFi != nullptr) {
