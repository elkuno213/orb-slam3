...
TumViRunner::TumViRunner(const RunConfig& config)
  ...
  , _inertial(_sensor == Sensor::InertialMono || _sensor == Sensor::InertialStereo)
  , _is_stereo(_sensor == Sensor::Stereo || _sensor == Sensor::InertialStereo)
...
Sensor TumViRunner::sensor() const noexcept {
  return _sensor;
}
