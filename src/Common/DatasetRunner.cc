...
/// @brief Map string sensor + inertial flag to System::Sensor.
[[nodiscard]] System::Sensor parseSensor(const std::string& s, bool inertial) {
  if (s == "mono") {
    return inertial ? System::Sensor::IMU_MONOCULAR : System::Sensor::MONOCULAR;
  }
  if (s == "stereo") {
    return inertial ? System::Sensor::IMU_STEREO : System::Sensor::STEREO;
  }
  if (s == "rgbd") {
    if (inertial) {
      throw po::error("rgbd does not support inertial mode");
    }
    return System::Sensor::RGBD;
  }
...
void validateConfig(const RunConfig& config) {
  switch (config.dataset) {
    case DatasetType::EuRoC:
      if (config.sensor != System::Sensor::MONOCULAR && config.sensor != System::Sensor::IMU_MONOCULAR
          && config.sensor != System::Sensor::STEREO && config.sensor != System::Sensor::IMU_STEREO) {
        throw po::error("euroc only supports mono and stereo sensors (with optional inertial)");
      }
      break;
    case DatasetType::TUM:
      if (config.sensor != System::Sensor::MONOCULAR && config.sensor != System::Sensor::RGBD) {
...
void saveTrajectories(
  System& slam, const fs::path& output_dir, DatasetType dataset, System::Sensor sensor
) {
  ...
  switch (dataset) {
    ...
    case TumVI:
      ...
      break;
    case TUM:
      if (sensor == System::Sensor::MONOCULAR || sensor == System::Sensor::IMU_MONOCULAR) {
