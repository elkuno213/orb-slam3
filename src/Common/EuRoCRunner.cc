...
EuRoCRunner::EuRoCRunner(const RunConfig& config)
  ...
  , _inertial(config.sensor == System::Sensor::InertialMono || config.sensor == System::Sensor::InertialStereo)
  , _data_dirs(config.data_dirs) {
}

void EuRoCRunner::load() {
  const bool is_stereo = _sensor == System::Sensor::Stereo || _sensor == System::Sensor::InertialStereo;
