...
void KeyFrameDatabase::DetectCandidates(
  ...
  const auto           spConnectedKeyFrames = pKF->GetConnectedKeyFrames();
...
void KeyFrameDatabase::DetectNBestCandidates(
  ...
  while (
    ...
    if (!spAlreadyAddedKF.contains(pKFi)) {
      if (pKF->GetMap() == pKFi->GetMap()
          && vpLoopCand.size() < static_cast<std::size_t>(nNumCandidates)) {
        vpLoopCand.push_back(pKFi);
      } else if (pKF->GetMap() != pKFi->GetMap()
                 && vpMergeCand.size() < static_cast<std::size_t>(nNumCandidates)
                 && !pKFi->GetMap()->IsBad()) {
        vpMergeCand.push_back(pKFi);
      }
      spAlreadyAddedKF.insert(pKFi);
