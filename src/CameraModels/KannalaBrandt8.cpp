...
cv::Point2f KannalaBrandt8::project(const cv::Point3f& p3D) {
  ...
  const float theta      = std::atan2(std::sqrt(x2_plus_y2), p3D.z);
  const float psi        = std::atan2(p3D.y, p3D.x);
  ...
  return {
    static_cast<float>(mvParameters[0] * r * std::cos(psi) + mvParameters[2]),
    static_cast<float>(mvParameters[1] * r * std::sin(psi) + mvParameters[3])
  };
}

Eigen::Vector2d KannalaBrandt8::project(const Eigen::Vector3d& v3D) {
  const double x2_plus_y2 = v3D[0] * v3D[0] + v3D[1] * v3D[1];
  const double theta      = std::atan2(std::sqrt(x2_plus_y2), v3D[2]);
  const double psi        = std::atan2(v3D[1], v3D[0]);
...
Eigen::Vector2f KannalaBrandt8::project(const Eigen::Vector3f& v3D) {
  ...
  const float theta      = std::atan2(std::sqrt(x2_plus_y2), v3D[2]);
  const float psi        = std::atan2(v3D[1], v3D[0]);
...
Eigen::Matrix<double, 2, 3> KannalaBrandt8::projectJac(const Eigen::Vector3d& v3D) {
  ...
  const double theta = std::atan2(r, v3D[2]);
