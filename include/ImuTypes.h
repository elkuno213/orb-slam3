...
public:
  Calib();
  Calib(const Sophus::SE3f& Tbc, float ng, float na, float ngw, float naw);
  Calib(const Calib& calib);

  // void Set(const cv::Mat &cvTbc, const float &ng, const float &na, const float &ngw, const float
  // &naw);
  void Set(const Sophus::SE3f& sophTbc, float ng, float na, float ngw, float naw);
...
private:
  ...
  struct integrable {
    ...
    integrable() = default;
    integrable(Eigen::Vector3f a_, Eigen::Vector3f w_, float t_)
